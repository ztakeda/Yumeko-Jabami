...
module.exports = {
	...
	noLimit: true,
    start: async(yumeko, m, {isPremium, isOwner}) => {
		let statuses
		try {
			statuses = await yumeko.fetchStatus(m.sender);
		} catch {
			statuses = "Nothing.."
		}
		...
		try {
			var pp = await yumeko.profilePictureUrl(m.sender, "image");
		} catch {
		...
		caption += `│\n`
		caption += `└───────⭓\n`
		yumeko.sendFile(m.from, pp, "", m, { caption })
    }
}