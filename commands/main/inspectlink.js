...
module.exports = {
    ...
    start: async(yumeko, m, { text }) => {
        ...
        let res = await yumeko.groupQueryInvite(code)
...
⬡ *Friends Who Are Known to Join :*\n${res.participants ? res.participants.map((user, i) => ++i + ". @" + user.jid.split("@")[0]).join("\n").trim() : "Not Found"}
        ...
        let pp
        try {
            pp = await yumeko.profilePictureUrl(res.id, "image")
        } catch {
            pp = "https://tse2.mm.bing.net/th?id=OIP.n1C1oxOvYLLyDIavrBFoNQHaHa&pid=Api&P=0&w=153&h=153"
        }
        yumeko.sendFile(m.from, pp, "", m, { caption: teks, mentions: await yumeko.parseMention(teks) })
    },
    isQuery: true
}
