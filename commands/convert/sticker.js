...
module.exports = {
    ...
    start: async(yumeko, m, { command, prefix, text, quoted, mime }) => {
        ...
        if (/image|video|sticker/.test(mime)) {
            ...
            yumeko.sendFile(m.from, download, "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
        } else if (quoted.mentions[0]) {
            let url = await yumeko.profilePictureUrl(quoted.mentions[0], "image")
            yumeko.sendFile(m.from, url, "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
        } else if (isUrl(text)) {
            if (isUrl(text)) yumeko.sendFile(m.from, isUrl(text)[0], "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
            ...
            let download = await yumeko.downloadMediaMessage(message)
            yumeko.sendFile(m.from, download, "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
