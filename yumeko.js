...
module.exports = async (yumeko, m, commands, chatUpdate) => {
    try {
        ...
        let metadata = isGroup ? await yumeko.groupMetadata(from) : {}
        ...
        let isBotAdmin = isGroup ? groupAdmin.includes(yumeko.user?.jid) : false
        let isAdmin = isGroup ? groupAdmin.includes(sender) : false
        let isOwner = [yumeko.user?.jid, ...config.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(sender)
        ...
        user.expiredCheck(yumeko, m, _user);

        if (config.options.autoRead) (yumeko.type == "legacy") ? await yumeko.chatRead(m.key, 1) : await yumeko.sendReadReceipt(from, sender, [m.id])
        ...
        if (isMedia && m.msg.fileSha256 && (m.msg.fileSha256.toString("hex") in global.db.sticker)) {
            ...
            let messages = await generateWAMessage(m.from, { text, mentions }, {
                userJid: yumeko.user.jid,
                quoted: m.quoted && m.quoted.fakeObj
            })
            messages.key.fromMe = await areJidsSameUser(m.sender, yumeko.user.jid)
            ...
            yumeko.ev.emit("messages.upsert", msg)
        }
        ...
        if (isGroup && isBotAdmin && isAntilink && !isAdmin && !isOwner) {
            if (budy.match("://chat.whatsapp.com/")) {
                setTimeout( () => {
                    yumeko.groupParticipantsUpdate(from, [sender], "remove")
                }, 5 * 1000)
                setTimeout( () => {
        ...
        if (isAntidelete && m.message && m.message.protocolMessage && m.message.protocolMessage.type == 0) {
            ...
            let msg = await yumeko.serializeM(await Store.loadMessage(key.remoteJid, key.id))
            ...
            yumeko.relayMessage(m.from, msg.message, { messageId: msg.id })
            await yumeko.sendText(m.from, tekss, msg, { mentions: [msg.sender] })
        }
